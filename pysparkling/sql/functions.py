...
from pysparkling import since, SparkContext
from pysparkling.rdd import _prepare_for_python_RDD, ignore_unicode_prefix
from pysparkling.serializers import PickleSerializer, AutoBatchedSerializer
from pysparkling.sql.types import StringType, DataType, _parse_datatype_string
from pysparkling.sql.column import Column, _to_java_column, _to_seq
...
def to_date(col, format=None):
    """Converts a :class:`Column` of :class:`pysparkling.sql.types.StringType` or
    :class:`pysparkling.sql.types.TimestampType` into :class:`pysparkling.sql.types.DateType`
    using the optionally specified format. Specify formats according to
    `SimpleDateFormats <http://docs.oracle.com/javase/tutorial/i18n/format/simpleDateFormat.html>`_.
    By default, it follows casting rules to :class:`pysparkling.sql.types.DateType` if the format
...
def to_timestamp(col, format=None):
    ...
    using the optionally specified format. Specify formats according to
    `SimpleDateFormats <http://docs.oracle.com/javase/tutorial/i18n/format/simpleDateFormat.html>`_.
    By default, it follows casting rules to :class:`pysparkling.sql.types.TimestampType` if the format
...
def window(timeColumn, windowDuration, slideDuration=None, startTime=None):
    ...
    The time column must be of :class:`pysparkling.sql.types.TimestampType`.
    ...
    and 'end', where 'start' and 'end' will be of :class:`pysparkling.sql.types.TimestampType`.
...
def locate(substr, str, pos=1):
    ...
    :param str: a Column of :class:`pysparkling.sql.types.StringType`
...
@since(1.5)
def hex(col):
    """Computes hex value of the given column, which could be :class:`pysparkling.sql.types.StringType`,
    :class:`pysparkling.sql.types.BinaryType`, :class:`pysparkling.sql.types.IntegerType` or
    :class:`pysparkling.sql.types.LongType`.
...
def map_keys(col):
    ...
    >>> from pysparkling.sql.functions import map_keys
...
def map_values(col):
    ...
    >>> from pysparkling.sql.functions import map_values
    >>> df = spark.sql("SELECT map(1, 'a', 2, 'b') as data")
...
def pandas_udf(f=None, returnType=StringType()):
    ...
    >>> from pysparkling.sql.types import IntegerType, StringType
...
def _test():
    import doctest
    from pysparkling.sql import Row, SparkSession
    import pysparkling.sql.functions
    globs = pysparkling.sql.functions.__dict__.copy()
    ...
    (failure_count, test_count) = doctest.testmod(
        pysparkling.sql.functions, globs=globs,
