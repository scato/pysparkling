...
from pysparkling import since
from pysparkling.rdd import ignore_unicode_prefix
from pysparkling.sql.session import _monkey_patch_RDD, SparkSession
from pysparkling.sql.dataframe import DataFrame
from pysparkling.sql.readwriter import DataFrameReader
from pysparkling.sql.streaming import DataStreamReader
from pysparkling.sql.types import IntegerType, Row, StringType
from pysparkling.sql.utils import install_exception_handler
...
class SQLContext(object):
    ...
    def range(self, start, end=None, step=1, numPartitions=None):
        """
        Create a :class:`DataFrame` with single :class:`pysparkling.sql.types.LongType` column named
    ...
    def registerFunction(self, name, f, returnType=StringType()):
        ...
        :param returnType: a :class:`pysparkling.sql.types.DataType` object
    ...
    def createDataFrame(self, data, schema=None, samplingRatio=None, verifySchema=True):
        ...
        When ``schema`` is :class:`pysparkling.sql.types.DataType` or a datatype string it must match
        the real data, or an exception will be thrown at runtime. If the given schema is not
        :class:`pysparkling.sql.types.StructType`, it will be wrapped into a
        :class:`pysparkling.sql.types.StructType` as its only field, and the field name will be "value",
        ...
        :param schema: a :class:`pysparkling.sql.types.DataType` or a datatype string or a list of
            column names, default is None.  The data type string format equals to
            :class:`pysparkling.sql.types.DataType.simpleString`, except that top level struct type can
            omit the ``struct<>`` and atomic types use ``typeName()`` as their format, e.g. use
            ``byte`` instead of ``tinyint`` for :class:`pysparkling.sql.types.ByteType`.
            We can also use ``int`` as a short name for :class:`pysparkling.sql.types.IntegerType`.
        :param samplingRatio: the sample ratio of rows used for inferring
        :param verifySchema: verify data types of every row against schema.
        :return: :class:`DataFrame`

        .. versionchanged:: 2.0
           The ``schema`` parameter can be a :class:`pysparkling.sql.types.DataType` or a
           datatype string after 2.0.
           If it's not a :class:`pysparkling.sql.types.StructType`, it will be wrapped into a
           :class:`pysparkling.sql.types.StructType` and each record will also be wrapped into a tuple.
        ...
        >>> from pysparkling.sql import Row
        >>> Person = Row('name', 'age')
    ...
    def streams(self):
        ...
        .. note:: Evolving.
        """
        from pysparkling.sql.streaming import StreamingQueryManager
...
def _test():
    ...
    from pysparkling.sql import Row, SQLContext
    import pysparkling.sql.context

    os.chdir(os.environ["SPARK_HOME"])

    globs = pysparkling.sql.context.__dict__.copy()
    ...
    (failure_count, test_count) = doctest.testmod(
        pysparkling.sql.context, globs=globs,
